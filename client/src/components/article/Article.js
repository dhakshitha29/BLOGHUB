      ...
      {articleEditStatus === false ? (
        <>
          <div className="d-flex justify-content-between">
            <div>
              <p className="pt-4 display-4 me-4">{state.title}</p>
              <span className="py-3">
          ...
          <div>
                ...
                state.comments.map((commentObj, ind) => {
                  return (
                    <div key={ind} className="bg-light  p-3">
                      <p className="fs-4" style={{  color: "rgba(169,29,58)", textTransform: "capitalize" }}>
                        <FcPortraitMode className="fs-2 me-2" />
                        {commentObj.username}
                      </p>

                      <p style={{ color: "black"}} className="ps-4">
            ...
            {currentUser.userType === "user" && (
              <form onSubmit={handleSubmit(writeComment)}>
                <input type="text" {...register("comment")} className="form-control mb-4 " placeholder="Write comment here...."/>
